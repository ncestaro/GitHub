...
os.makedirs('work/v2/tmpHD',exist_ok=True)
out='finali_editoriale_HD'; shutil.rmtree(out,ignore_errors=True); os.makedirs(out)
for n,src,name in SEQ:
    if n=='09': src='work/full/IMG_0617.jpg'
    im=Image.open(src).convert('RGB'); W,H=im.size
    if W/H>0.75: w=int(H*0.75); im=im.crop(((W-w)//2,0,(W-w)//2+w,H))
    else: h=int(W/0.75); im=im.crop((0,(H-h)//2,W,(H-h)//2+h))
    im=im.resize((1800,2400),Image.LANCZOS); tmp=f'work/v2/tmpHD/{n}.png'; im.save(tmp)
